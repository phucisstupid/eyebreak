...
final class MenuBarContentViewTests: XCTestCase {
    func test_menuContentReflectsWaitingForIdleState() {
        ...
        let model = AppModel.makeForTests(coordinator: DummyAppCoordinator(), snapshot: snapshot, settings: settings)
    ...
    func test_quitCommandInvokesClosure() {
        let model = AppModel.makeForTests(coordinator: DummyAppCoordinator())
