...
final class MenuBarContentViewTests: XCTestCase {
    func test_menuContentReflectsWaitingForIdleState() {
        ...
        let model = AppModel.makeForTests(coordinator: SpyAppCoordinator(settings: .default, snapshot: .waitingForIdle(progress: 0, breakCount: 0, nextBreakType: .short)), snapshot: snapshot, settings: settings)
    ...
    func test_quitCommandInvokesClosure() {
        let model = AppModel.makeForTests(coordinator: SpyAppCoordinator(settings: .default, snapshot: .waitingForIdle(progress: 0, breakCount: 0, nextBreakType: .short)))
